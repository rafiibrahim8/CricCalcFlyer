...
public class PlayerNamesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        file = new File(getFilesDir()+"/playerNames.json");
        boolean fileExist = file.exists();
        json = null;
        if (fileExist){
            try{
                FileInputStream fis = new FileInputStream(file);
                json = new JSONObject(new String(IOUtils.toByteArray(fis),"UTF-8"));
                fis.close();
            } catch(Exception ex){}
        ...
        for(int i=0;i<11;i++){
            ...
            player[i].setHintTextColor(getResources().getColor(R.color.androidHint));
